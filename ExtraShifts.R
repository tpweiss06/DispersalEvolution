...
SimFunc <- function(i){
     SimID <- strsplit(x = as.character(ReSampledSims$ID[i]), split = "/")[[1]][4]
     Survival <- ExtraShift(SimDir = SimID, parallel = TRUE, NewSpeed = IncreasedSpeed,
...
# Use that to create a new data frame in which the original, equilibrium simulations
#    are resampled (with replacement) 1,000 times to create more replicates for the
#    range shift simulations. The new data.frame will have a column for the number
#    of loci ("L") and the simulation ID ("ID"), just like the original SimData 
#    data frame
ReSampledSims <- data.frame(L = rep(Lseq, each = NumSims), ID = rep(NA, length(Lseq) * NumSims))
for(l in 1:length(Lseq)){
     CurSims <- which( (SimData$L == Lseq[l]) )
     ReSampledSims$ID[((l-1)*NumSims + 1):(l*NumSims)] <- sample(as.character(SimData$ID[CurSims]), size = NumSims, replace = TRUE)
}
# Create a vector of parameter index values for the parallel computation
SimVec <- 1:nrow(ReSampledSims)

# Create the cluster and run the simulations
cl <- makeCluster(nProc - 1, type = "MPI")

# Export the necessary objects to each node
clusterExport(cl, c("IncreasedSpeed", "ReSampledSims") )
...
for(l in 1:length(Lseq)){
     CurSims <- which(ReSampledSims$L == Lseq[l])
     TotalExtinct <- 0
     for(i in 1:NumSims){
          TotalExtinct <- TotalExtinct + Sims[[CurSims[i]]]
     }
     ExtProb[l] <- TotalExtinct / NumSims
}
