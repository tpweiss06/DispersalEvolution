...
AllSimulations <- read.csv("SensSimulations.csv")
...
SimFunc <- function(i){
     ...
     if(AllSimulations$complete[i] == 0){
          ...
          CurU <- AllSimulations$U[i]
          CurSigma <- AllSimulations$sigma[i]
          CurParams <- list(BetaInit, gamma, tau, CurOmega, L, R, Kmax, 
          ...
          SimDir <- "/project/rangeecoevomodels/cweissle/DispEv/SensSims"
