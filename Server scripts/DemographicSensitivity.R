...
PartialParams <- c("BetaInit", "gamma", "tau", "U", "sigma", "kern", 
                   "BurnIn", "LengthShift", "ExpandGens", "psi", "DispVar",
                   "dmax", "rho", "lambda", "NumRands", "v")


# Load in the master table of simulations to run
AllSimulations <- read.csv("DemoSensSimulations.csv")
...
SimFunc <- function(i){
     ...
     if(AllSimulations$complete[i] == 0){
          ...
          CurR <- AllSimulations$R[i]
          CurKmax <- AllSimulations$Kmax[i]
          CurParams <- list(BetaInit, gamma, tau, CurOmega, U, sigma, L, CurR, CurKmax, 
                            CurHap, kern, CurMon, BurnIn, LengthShift, ExpandGens, 
                            psi, DispVar, dmax, rho, lambda, NumRands, v)
          ...
          SimDir <- "/project/rangeecoevomodels/cweissle/DispEv/DemoSensSims"
